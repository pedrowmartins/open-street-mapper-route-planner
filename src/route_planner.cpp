...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    RouteModel::Node* node_lowest_f_value = open_list[0];
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    if (current_node == (this->end_node)){ //current node is the end node
        while (current_node != (this->start_node)){ //while the path hasnt been regressed back to the start_node
        ...
        std::sort(path_found.begin(),path_found.end(),[&](RouteModel::Node a, RouteModel::Node b){
            return (a.distance(*(this->start_node)) < b.distance(*(this->start_node)));
        });
    }
